document.addEventListener('DOMContentLoaded', () => {
  ...
  const daysCount = {{ plan.day_variants|length }}; // Jinja-инъекция
  ...
  btn.addEventListener('click', async () => {
    const choices = [];
    for (let d = 1; d <= daysCount; d++) {
      choices.push(document.querySelector(`input[name="day${d}"]:checked`).value);
    }
    ...
    const res = await fetch('/api/session', {
      ...
      body: JSON.stringify({routeId, checkIn})
    });
