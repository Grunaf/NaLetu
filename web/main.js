  ...
  document.getElementById('toDetailsBtn').onclick = () => {
    ...
    renderItinerary(window.itinerary); // ВЫЗЫВАТЬ здесь!
  };
  ...
  async function loadPlan() {
    try {
      ...
      const res = await fetch(`http://127.0.0.1:3000/api/plan/${routeId}`);
  ...
  function renderStep1(itinerary) {
    ...
    document.getElementById('castVotePOI').onclick = async () => {
      const choice = itinerary.day_variants.map(d =>
        document.querySelector(`input[name="day${d.day}"]:checked`).value);
      ...
      showModal(`
        ...
        m => {
          ...
          m.querySelector('#expOk').onclick     = () => {
            ...
            window.open(`/api/session/${window.itinerary.session_id}/export?format=${fmt}`, '_blank');
    ...
    btnInvite.onclick = () => {
      ...
      showModal(`
        ...
        <footer>
          ...
          m.querySelector('#closeInvite').onclick = () => m.parentElement.remove();
        });
    };
  ...
  function renderItinerary(itinerary) {
    ...
    const chosenIds = itinerary.day_variants.map(d =>
    document.querySelector(`input[name="day${d.day}"]:checked`).value);
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (Array.isArray(result)) {
      itinerary = result.find(x => String(x.id || x.session_id) === String(routeId));
    } else if (result && (result.id || result.session_id)) {
      itinerary = (!routeId || String(result.id || result.session_id) === String(routeId)) ? result : null;
    }
    if (!itinerary) {
      document.getElementById('app').innerHTML = '<div class="error">Маршрут не найден</div>';
      return;
    }
    window.itinerary = itinerary;
    renderStep1(itinerary);
    // 3) Найти кнопки и секции
    const step1El = document.getElementById('step1');
    const step2El = document.getElementById('step2');
