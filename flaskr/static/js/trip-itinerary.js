  ...
  const observer = new IntersectionObserver(
    ([entry]) => {
      if (entry.isIntersecting) {
        budgetBlock.classList.remove("border-t");
        budgetBlock.classList.remove("mt-6");
        budgetBlock.classList.remove("shadow");

        expancesBlock.classList.remove("hidden");
      } else {
        budgetBlock.classList.add("border-t");
        budgetBlock.classList.add("mt-6");
        budgetBlock.classList.add("shadow");
...
function parseMealPlaceInfo(input) {
  const name = input.dataset.name;
  const imgSrc = input.dataset.imgSrc;
  const addressName = input.dataset.addressName;
  const generalRating = input.dataset.generalRating;
  const generalReviewCount = input.dataset.generalReviewCount;
  const avgPrice = input.dataset.avgPrice;
  ...
  return {
    ...
    general_review_count: generalReviewCount,
    avg_price: avgPrice,
  };
}
function updateMealPlaceCard(main_input, spot_info) {
  const blockMealPlace = main_input.nextElementSibling;
  const desc = blockMealPlace.firstElementChild.firstElementChild;
  const img = blockMealPlace.lastElementChild;

  const attributesContent = `<div class="flex">
                                      <i class="material-icons-round">star</i>${spot_info["general_rating"]}  
                                  </div> ${spot_info["general_review_count"]} отзывов ${spot_info["avg_price"]} ₽ ${spot_info["address_name"]}`;
  const attributesBlock = desc.lastElementChild;
  const headerBlock = desc.firstElementChild;

  headerBlock.innerHTML = spot_info["name"];
  attributesBlock.innerHTML = attributesContent;
  img.src = spot_info["img_src"];
}
document.addEventListener("DOMContentLoaded", () => {
  ...
  simularSpots.addEventListener("click", event => {
    ...
    if (target.matches("input[type='radio'][data-cost]")) {
      ...
      const spotInfo = parseMealPlaceInfo(target);
      updateMealPlaceCard(input, spotInfo);
      updateBudgetBlock(input, dayOrder);
    }
  });
...
function createExpanceBlock(option, option_name, dayOrder) {
  ...
  return `<div class="flex gap-3">
      <div class="bg-gray-200 rounded-3xl size-10 flex justify-center items-center" data-input-name="${option_name}">
...
function addOrUpdateExpanceBlock(option, dayOrder) {
  ...
  if (existExpanceBlock) {
    ...
    expancesBlock.insertAdjacentHTML("beforeend", newExpanceBlock);
  }
}
